...
// // Fonction de debug
// void printCaddie(CaddieArticle caddie[10]);//fonction de correction preter par lucas
...
char* OVESP_Caddie(struct CaddieArticle caddie[10]) {
    ...
    for (int i = 0; i < 10; ++i) {
        if (caddie[i].idArticle != -1) {
            ...
            strcat(article, temp);// La chaîne temp est concaténée à la chaîne article à l'aide de la fonction strcat, ce qui ajoute les informations de l'article actuel à la fin de la chaîne article
